...
func (c *ReflectorsController) startNamespaceReflection(namespace string) {
	...
	if err := c.cacheManager.AddNamespace(namespace, nattedNs); err != nil {
	...
	go func() {
		if err := c.cacheManager.StartNamespaces(namespace, nattedNs, c.namespacedStops[namespace]); err != nil {
